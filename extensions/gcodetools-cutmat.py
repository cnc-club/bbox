...
TURN_KNIFE_ANGLE_TOLERANCE = 1e-3 # in radians - tolerance on which we should get tangent knife up tu turn it 
...
###
############################################################################
def csp_vector_intersection(l1,l2,sp1,sp2):
	dd=l1[0]
	cc=l2[0]-l1[0]
	bb=l1[1]
	aa=l2[1]-l1[1]
	...
	for i in roots :
		if type(i) is complex and abs(i.imag)<1e-7:
			i = i.real
		if type(i) is not complex and -1e-10<=i<=1.+1e-10:
			ix = ax*(i**3)+bx*(i**2)+cx*i+x0
			iy = ay*(i**3)+by*(i**2)+cy*i+y0

			if (cc == 0) or (cc > 0 and ix-l2[0]>0) or (cc < 0 and ix-l2[0]<0):
				if (aa == 0) or (aa > 0 and iy-l2[1]>0) or (aa < 0 and iy-l2[1]<0):
					retval.append(i)
	return retval
...
###
############################################################################
	def box_cutter_autoreverse(self) :
		if self.selected_paths == {} and self.options.auto_select_paths:
			self.selected_paths = self.paths
		...
		for layer in self.layers :
			if layer in self.selected_paths :
				for path in self.selected_paths[layer]:
					csp = CSP(path)
					for sp in csp.items :
						# tol`ko zakry`ty`e kontury`
						if sp.is_closed() :
							i = 0
							while i < len(sp.points)-1 :
								# normal` (vektor) k krivoi` v eyo nachale. napravlen vlevo ot napravleniia obhoda
								n1 = sp.normal(i, 0)
								#self.error(_("normal  %f  %f" % (n1.x, n1.y)), "warning")
								v1 = [sp.points[i][1].x, sp.points[i][1].y]
								v2 = [sp.points[i][1].x + n1.x, sp.points[i][1].y + n1.y]								
								v3 = [sp.points[i][1].x - n1.x, sp.points[i][1].y - n1.y]								
								
								crosses = 0
								
								j = 0
								while j <= len(sp.points)-1 :
									j1 = j-1
									j2 = j;
									
									if (sp.points[j1][1].x == sp.points[j2][1].x) and (sp.points[j1][1].y == sp.points[j2][1].y) :
										j += 1
										continue
									
									# schitaem peresecheniia
									if len(csp_vector_intersection(v1, v2, sp.cp_to_list(j1), sp.cp_to_list(j2))) > 0 :
										# est` peresechenie => e`ta tochka ne podhodit
										crosses = 1
										break
								...
								# net peresecheniia
								# esli peresecheniia u normali v dannoi` tochke net, to nuzhno proverit` i v druguiu storonu
								# esli v tu est`, to  kontur napravlen verno
								if crosses == 0 :
									crosses = 1
									#self.error(_("anti-cross check"), "warning")
									j = 0
									while j <= len(sp.points)-1 :
										j1 = j-1
										j2 = j;
										...
										if len(csp_vector_intersection(v1, v3, sp.cp_to_list(j1), sp.cp_to_list(j2))) > 0 :
											# est` peresechenie => vsyo ok
											crosses = 0
											break
							...
							self.error(_("Found unclosed path!"),"warning")

					# delete the original path
					parent = path.getparent()
					csp.draw(group=parent, style_from=path, gcodetools_tag="")
					parent.remove(path)
		
		self.error(_("Done. Reversed paths: %d" % done), "warning")
...
###
############################################################################
	def box_cutter_circlefix(self) :
	
		rr = 0.5 # small radius | original radius
		dl = 1 # dlina dugi hvostika
		tools_for_preparation = ['tangent knife'] # ���� ��� ������� ����� ������ ���������� (������������ ������������)

		#import pprint
		#pp = pprint.PrettyPrinter(indent=4, depth=6)

		def is_layer_visible(lay) :
			if lay.get('style') is None :
				return True
			return ('display:inline' in lay.get('style').lower())

		def is_need_preparation(lay) :
				if lay not in self.tools :
					return False
				#self.set_tool(lay) # ???
				#self.error(pp.pformat(self.tools[lay]), "warning")
				return (self.tools[lay][0]['name'].lower() in tools_for_preparation)

		def get_startend_info(curve) :
			res = [None,None]
			for fragment in curve :
				if fragment[1] in ["line","arc"] and point_to_point_d2(fragment[0],fragment[4]) > 1 :
					if res[0] is None :
						res[0] = fragment
					res[1] = fragment
			i = 0
			for s in res :
				if s[1] == "arc" :
					res[i] = [point_to_point_d(s[0],s[4]), point_to_point_d(s[0],s[2])]
					# length?, radius
				else :
					res[i] = [point_to_point_d(s[0],s[4]), 10000]
					# line as circle with 10m radius
				i += 1
		...
		# Rabotaem tol`ko s vidimy`mi sloiami (master-sloi` tozhe ignoriruetsia)
		#for layer in self.layers :
		#	if is_layer_visible(layer) :			
				##self.error(self.layers[1].get('style'), "warning")
		...
		for layer in self.layers :
			...
			#	self.error(_("visi: "+pp.pformat(layer.values())),"warning")
			#else :
			#	self.error(_("non visi: "+pp.pformat(layer.values())),"warning")

			if not is_need_preparation(layer) :
				continue
			...
			if layer in self.paths :
				#for path in self.selected_paths[layer] :
				for path in self.paths[layer] :
					#self.error("path", "warning")
					#self.error(pp.pformat(self.layers[1].get('style')), "warning")
					csp = CSP(path)
					res = CSP([])
					
					newcsp = cubicsuperpath.parsePath(path.get("d"))
					#self.error(pp.pformat(newcsp), "warning")
					#curve = [ self.parse_curve([subpath], layer) for subpath in newcsp  ]
					
					spi = 0
					for sp in csp.items :
						#self.error("sp", "warning")
						
						#curve = gcodetools.parse_curve([sp], layer)
						#curve=self.parse_curve([sp], layer)
						
						#curve = sp.get("d")
						#self.error(pp.pformat(sp[0]), "warning")
						
						# obrabaty`vaem tol`ko "zakry`ty`e" kontury`
						if sp.is_closed() :

							#self.error("found", "warning")
							nbeg,nend = sp.normal(0,0), sp.normal(-1,1)
							#self.error(pp.pformat([nbeg,nend]), "warning")
							a = nend.angle()-nbeg.angle()
							# esli kontcy` shodiatsia pod 180 gradusov (+-1 gradus)
							if abs(a) < 0.02 :
								# sozdaem na kazhdom iz kontcov po odnomu  koroten`komu arku
								#self.error(_("Angle = %s" % (a)), "warning")

								startend = get_startend_info( self.parse_curve([newcsp[spi]], layer) )
								#self.error(pp.pformat(startend), "warning")

								# Dobavliaem novuiu tochku v nachalo
								r_small = startend[1][1]*rr
								alfa = dl / (2*r_small)
								x = 2*r_small * sin(alfa)
								nx = nend.rot(pi/2+alfa)

								st = sp.points[0][1]
								sp.points[0][0] = st.copy()
								st = st + nx*x
								sp.points = [ [st.copy(),st.copy(),st.copy()] ] + sp.points
								# meniaem ry`chag by`vshego nachala na kasatel`ny`i`
								sp.points[1][0] = sp.points[1][0] + nend.ccw()*(x/2)

								# Konetc. perpendikuliar k kasatel`noi` vpravo (2mm)
								r_small = startend[0][1]*rr
								alfa = dl / (2*r_small)
								x = 2*r_small * sin(alfa)
								nx = nbeg.cw().rot(-alfa)
								#self.error(_("alfa = %s      r_small = %s" % (alfa, r_small)), "warning")
								...
								# meniaem ry`chag
								sp.points[-2][2] = sp.points[-2][2] + nbeg.cw()*(x/2)
							#else :
							#	self.error(_("not Pi! normal' angle = %s" % (a)), "warning")

						res.items.append(sp)
						spi += 1

					# udaliaem stary`i` i otrisovy`vaem modifitcirovanny`i` put`
					parent = path.getparent()
...
#					s = 1. if "stroke-width" not in st else float(st["stroke-width"])*25.4/90.
#					s = s/2+0.125
					depths = [max(-0.125, (self.options.box_prepare_a-0.125)*(100-self.options.box_prepare_e)/100.   )]
				elif self.tools[layer][0]['id'][:3].lower() == "pen" : 	
					depths = [21.]
				else : 
					depths = [-0.125] # TODO: to dialog
				return depths

		
################################################################################
###		Cutmat main function 
###		
###		
################################################################################
	def cutmat(self):
		if self.selected_paths == {} and self.options.auto_select_paths:
			self.selected_paths = dict([(layer,self.paths[layer][:]) for layer in self.paths])
			self.error(_("No paths are selected! Trying to work on all available paths."),"warning")
		if self.selected_paths == {}:
			self.error(_("Nothing is selected. Please select something."),"warning")
			return
		...
		unselect = []			
		select = []
		# prepare V rows (канавки)
		for layer in self.layers :
			if layer.attrib["id"] == "layer4-9":
				if layer in self.selected_paths :
					for path in self.selected_paths[layer] :
						csp = CSP(path)
						csp_r = csp.copy()
						csp_r.reverse()
						csp.join(csp_r)
						p = csp.draw(group=layer, style_from=path)
						self.paths[layer].append(p)
						p.set("gcodetools-vrows","1")
						select.append(p)
						# now unselect prepared path's originals:
						unselect.append(path)							
					# select new paths
					self.selected_paths[layer]+=select

		# now unselect prepared path's originals:
		for path in unselect:
			for layer in self.selected_paths :
				self.selected_paths[layer] = [path for path in self.selected_paths[layer] if path not in unselect]

		# now to prepare paths
		...
		for layer in self.paths :
			select = []
			for path in self.paths[layer]:
				if path.get("gcodetools") == "Prepared original path" :
					path.set("gcodetools","")
					unselect.append(path)
				if path.get("gcodetools") == "Prepared path" :
					select.append(path)
			if select != [] :
				if layer in self.selected_paths :
					self.selected_paths[layer] += select
		...
		# now we can cut everything
		self.path_to_gcode()		

		# now delete everything
		for layer in self.paths : 
			for path in self.paths[layer] :
				if path.get("gcodetools") == "Prepared path" or path.get("gcodetools-vrows") == "1" : 
					path.getparent().remove(path)

		# now run linuxcnc!
		import subprocess
		if self.options.run_linuxcnc : 
			subprocess.Popen(["nohup", "linuxcnc", "~/linuxcnc/configs/cutmat.ini"]) 
...
################################################################################
	def box_cutter_prepare_path(self) :
		...
		# basic parameters
		beta = radians(32 + 50/60.0)
		fi = radians(53)

		# calibration parameters
		calibrate_thick = self.options.box_prepare_b
		deep = self.options.box_prepare_c * sin(fi) * cos(beta)

		# variables
		thick = self.options.box_prepare_a
		linear_d = self.options.box_prepare_d
		#hand_cut = self.options.hand_cut
		#normal_cut = (self.options.box_prepare_kind == 1)

		oc = ((calibrate_thick+deep) / sin(fi)) * (1/cos(beta) - (tan(beta))/(2*sin(beta)))
		wxy = thick / tan(fi)
		ah = thick / sin(fi)
		ac = (thick + deep) / sin(fi)
		bc = ac / cos(beta)
		ab = ac * tan(beta)
		ob = bc - oc
		o1b = ob * sin(beta)
		...
		def get_inout_adds(isin, isouter, a) :
			#if isin :
			#	warn("isin=1")
			#else :
			#	warn("isin=0")
			...
			alpha = a % (2*pi)
			if a < 0.5/180.*pi :  # corner less than 0.5 deg - do not process it
				return 0.
			if isouter : alpha = 2*pi - alpha
			#self.error(alpha/2, isin, isouter))
			fe1 = wxy / tan(alpha/2)
			gamma = atan(ah/fe1)
			o1e = ob * cos(beta) * tan(pi/2-gamma)

			if not isin : o1e = -o1e
			if isouter : o1e = -o1e

			# Pro znak:
			#    > 0 esli nozju nuzjno projti PO HODU eto rastoyanie do tochki
			#    < 0 esli nozju nuzjno projti OBRATNO eto rastoyanie do tochki			
			eb = o1e - o1b
			fg = -og * eb / ob
			hf = fg + hg
			ae = o1e + ao1
			#warn("a=%.3f, alpha=%.3f, eb=%.3f, fg=%.3f, hf=%.3f, ae=%.3f" % (a, alpha, eb, fg, hf, ae))
			...
			# 1			1		0
			# xor, =
			#if normal_cut ^ isouter :
			
			# Pro znak vozvrashaemogo znacheniya:
			#  *  return < 0  =  dobavit' fragment
			#  *  return > 0  =  otrezat' fragment
			if isouter :
				if isin :
					return (-fg - linear_d)
				else :
				...
				if isin :
					return (-max(eb, fg) - linear_d)
				else :
					return (ae + linear_d)



		corners_tolerance = 179.5
		corners_tolerance_inside = 179.5
		corners_tolerance_0 = 0.5
		tolerance = [cos((180-corners_tolerance)*pi/180),cos((180-corners_tolerance_inside)*pi/180), sin(corners_tolerance_0*pi/180)]
		...
		for layer in self.layers :
			if layer in self.selected_paths :
				...
				# ignore if don't have tool or orientation points
				if layer not in self.orientation_points : continue
				# ignore if that layer is not for cutting
				if self.tools[layer][0]['4th axis meaning'] != "tangent knife" : continue
...
#							#warn(n1.cross(n2),n1.dot(n2)) 
							#warn(n1.dot(n2), tolerance[0], tolerance[1])
							
							if	(
									(n1.cross(n2)<0 and n1.dot(n2)<tolerance[0] or  n1.cross(n2)>=0 and abs(n1.dot(n2))<tolerance[1]) and
									abs(n1.cross(n2)) > tolerance[2] 
								): 
								#draw_pointer(sp.points[i][1],size=100)

								a = pi + n2.angle() - n1.angle()
								#if n1.cross(n2) < 0 :
								#	warn(_("Outside Angle = %s" % (degrees(a))))
								#else :
								#	warn(_("Inside Angle = %s" % (degrees(a))))

								if n1.cross(n2) < 0 :
									l = get_inout_adds(True, False, a)
								else :
									l = get_inout_adds(True, True, a)
								#warn("l = %.4f" % l)
								...
								if n1.cross(n2) < 0 :
									l = get_inout_adds(False, False, a)
								else :
									l = get_inout_adds(False, True, a)
								#warn("l = %.4f" % l)
						...
						res.items.append(sp)
					parent = path.getparent()
					path.set("gcodetools", "Prepared original path")
					p = res.draw(group=parent, stroke="#005577", fill="none", gcodetools_tag="")
					p.set("gcodetools", "Prepared path")
					self.paths[layer].append(p)
...
#					poly.draw(color = "Violet",width=4)					
	...
	def __init__(self):
		...
		self.OptionParser.add_option("-d", "--directory",					action="store", type="string", 		dest="directory", default="~/",					help="Directory for gcode file")
		...
		self.OptionParser.add_option("",   "--run-linuxcnc",		action="store", type="inkbool", 		dest="run_linuxcnc", default=False,	help="Run linuxcnc after export.")
		self.OptionParser.add_option("",   "--box-prepare-a",		action="store", type="float", 		dest="box_prepare_a", default=10.,	help="See inx-file.")
		self.OptionParser.add_option("",   "--box-prepare-b",		action="store", type="float", 		dest="box_prepare_b", default=10.,	help="See inx-file.")
...
################################################################################
	...
	def check_dir(self):
		if self.options.run_linuxcnc : 
			self.options.add_numeric_suffix_to_filename = False
			self.options.file = "~/output.ngc"
...
###
################################################################################
	def generate_gcode(self, curve, layer, depths):
		...
		# TODO: check if that curve have corners
		one_depth = False
		current_a = None
		g_ = ""
		if len(depths) > 100 :
			for i in range(1, len(curve)) :
				...
				if s[1] != 'arc' :
					one_depth = True
					break
				current_a, axis4, g_ =  get_tangent_knife_turn_gcode(s,si,tool,current_a,0,0)
		...
		for zi in range(len(depths)):
			...
			#	Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
				...
				#else :
				#	g += "# p2p = %f\n" % point_to_point_d2(s[0],si[0])
				
				feed = f if lg not in ['G01','G02','G03'] else ''
				if s[1]	== 'move':
					if not ((zi > 0) and (i == 1) and point_to_point_d2(si[0], curve[len(curve)-1][0]) < 1e-7) :
						# only first time
						g += go_to_safe_distance + "G00" + c(si[0]) + "\n" + tool['gcode before path']
						g += "(Subpath start)\n"
						lg = 'G00'
						# TODO: ili ne ignorirovat' pravilnee?
				elif s[1] == 'end' :
					if zi == len(depths)-1 :
						g += "(Subpath end)\n"
					...
					if tool['4th axis meaning'] == "tangent knife" : 
						current_a, axis4, g_ =  get_tangent_knife_turn_gcode(s,si,tool,current_a, depths[zi], penetration_feed)
						g+=g_
					if lg=="G00": g += "G01" + c([None,None,s[5][0]+depths[zi]]) + penetration_feed +" (Penetrate before line)\n"	
					g += "G01" +c(si[0]+[s[5][1]+depths[zi]]) + feed + "\n"
					lg = 'G01'
				elif s[1] == 'arc':
					...
					if lg=="G00": g += "G01" + c([None,None,s[5][0]+depths[zi]]) + penetration_feed + " (Penetrate before arc)\n"				
					if (r[0]**2 + r[1]**2)>self.options.min_arc_radius**2:
						r1, r2 = (P(s[0])-P(s[2])), (P(si[0])-P(s[2]))
						if abs(r1.mag()-r2.mag()) < 0.001 :
							g += ("G02" if s[3]<0 else "G03") + c(si[0]+[ s[5][1]+depths[zi], (s[2][0]-s[0][0]),(s[2][1]-s[0][1])  ]) + feed + axis4 + "\n"
						else:
							r = (r1.mag()+r2.mag())/2
							g += ("G02" if s[3]<0 else "G03") + c(si[0]+[s[5][1]+depths[zi]]) + " R%f" % (r) + feed  + axis4 + "\n"
						lg = 'G02'
					else:
						if tool['4th axis meaning'] == "tangent knife" : 
							current_a, axis4, g_ = get_tangent_knife_turn_gcode(s[:1]+["line"]+s[2:],si,tool,current_a, depths[zi], penetration_feed)	
							g+=g_
						g += "G01" +c(si[0]+[s[5][1]+depths[zi]]) + feed + "\n"
						lg = 'G01'
	...
	def path_to_gcode(self):
		...
		for layer in self.layers :
			if layer in paths :
				...
				### TODO: Fix it!
				#if self.tools[layer][0]['4th axis meaning'] == "tangent knife" :
				for path in paths[layer] :
					if self.options.active_tab == '"cutmat"':
						depths = self.cutmat_get_depths(layer, path)
					d = getDfrompath(path)
					if d == None:
...
################################################################################
	def orientation(self, layer=None) :
		...
		if self.options.orientation_points_count == "in-out reference point" :
...
################################################################################
	...
	def fix_directory_path(self) :
		if self.options.directory[-1] not in ["/","\\"]:
			if "\\" in self.options.directory :
				self.options.directory += "\\"
			else :
				self.options.directory += "/"
		self.options.directory = os.path.expanduser(os.path.expandvars(self.options.directory))
...
################################################################################
	def effect(self) :
		...
		###########
		#	Cutmat hack 
		self.options.active_tab = '"cutmat"'
		...
		if self.options.active_tab == '"help"' :
		...
		elif self.options.active_tab not in ['"cutmat"', '"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"orientation"', '"tools_library"', '"offset"', '"arrangement"', '"update"', '"box-prepare-path"', '"ignore"']:
			self.error(_("Select one of the action tabs - Path to Gcode, Area, Import, Orientation, Offset or Tools library.\n Current active tab id is %s" % self.options.active_tab),"error")
		else:
			# Get all Gcodetools data from the scene.
			self.get_info()
			if self.options.active_tab in ['"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"box-prepare-path"']:
			...
			if self.options.active_tab == '"path-to-gcode"': 
				...
				self.box_cutter_prepare_path()		
			elif self.options.active_tab == '"box-cutter-autoreverse"': 
				self.box_cutter_autoreverse()
			elif self.options.active_tab == '"box-cutter-circlefix"': 
				self.box_cutter_circlefix()
